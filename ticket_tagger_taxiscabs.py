...
def tag_ticket(subcategory, subject):
    """Tags a Taxis/Cabs ticket"""
    subject_lower = subject.lower()
    
    if re.search(r'\b(bill|settlement|payment)\b', subject_lower):
        return 'Taxis/Cabs - Bill Settlement'
    else:
        return 'Taxis/Cabs - General Issue'

def generate_taxiscabs_pdf(save_dir, csv_path):
    ...
    plt.title('Taxis/Cabs Ticket Tag Distribution')
    pie_chart_path = f"{save_dir}/taxiscabs_pie_chart.png"
    ...
    pdf_path = f"{save_dir}/taxiscabs_report.pdf"
    doc = SimpleDocTemplate(pdf_path, pagesize=A4)
    styles = getSampleStyleSheet()
    elements = [
        Paragraph("Taxis/Cabs Ticket Tag Report", styles['Title']),
        Spacer(1, 12),
