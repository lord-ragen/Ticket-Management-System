...
def tag_ticket(subcategory, subject):
    ...
    if re.search(r'\b(error|fail|transaction)\b', subject_lower):
        return 'SWIFT - Error'
    else:
        return 'SWIFT - General Issue'

def generate_swift_pdf(save_dir, csv_path):
    ...
    plt.title('SWIFT Ticket Tag Distribution')
    pie_chart_path = f"{save_dir}/swift_pie_chart.png"
    ...
    pdf_path = f"{save_dir}/swift_report.pdf"
    doc = SimpleDocTemplate(pdf_path, pagesize=A4)
    styles = getSampleStyleSheet()
    elements = [
        Paragraph("SWIFT Ticket Tag Report", styles['Title']),
        Spacer(1, 12),
