...
def tag_ticket(subcategory, subject):
    ...
    if re.search(r'\b(error|fail|abort|issue)\b', subject_lower):
        return 'EOD - Error'
    else:
        return 'EOD - General Issue'

def generate_eod_pdf(save_dir, csv_path):
    ...
    plt.title('EOD Ticket Tag Distribution')
    pie_chart_path = f"{save_dir}/eod_pie_chart.png"
    ...
    pdf_path = f"{save_dir}/eod_report.pdf"
    doc = SimpleDocTemplate(pdf_path, pagesize=A4)
    styles = getSampleStyleSheet()
    elements = [
        Paragraph("EOD Ticket Tag Report", styles['Title']),
        Spacer(1, 12),
